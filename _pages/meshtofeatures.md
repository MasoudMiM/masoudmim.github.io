...
<div class="row text-center mt-4 mb-2">
  ...
  <div class="col-md-6 mb-3">
    ...
    <p class="mt-2"><small class="text-muted">Now in the official FreeCAD addon index — bug reports with the STL attached are welcome.</small></p>
  </div>
</div>
...
<div class="row mt-4">
  <div class="col-md-12">
    <h2>Install</h2>
    <p>
      In FreeCAD (1.1+), open <strong>Tools → Addon Manager</strong>,
      search for <em>MeshToFeatures</em>, install, and restart — the addon
      is in the official FreeCAD addon index.  (Developers can instead add
      <code>https://github.com/MasoudMiM/MeshToFeatures</code> as a custom
      repository to track <code>main</code> directly.)  The geometry core
      needs a few Python packages inside FreeCAD's interpreter (numpy,
      scipy, trimesh, shapely, rtree) — the Addon Manager offers to install
      them, but sandboxed builds such as Flatpak need a manual
      <code>pip</code> step; the
      <a href="https://github.com/MasoudMiM/MeshToFeatures/blob/main/freecad/meshtofeatures_wb/docs/VERIFY.md"
         target="_blank" rel="noopener">verification guide</a> has the exact
